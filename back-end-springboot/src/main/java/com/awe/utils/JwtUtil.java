...
public class JwtUtil {
    ...
    /**
     * 生成jtw
     * @param uuid token中要存放的数据（json格式）
     * @return
     */
    public static String createJWT(String uuid) {
        JwtBuilder builder = getJwtBuilder(null, uuid);// 设置过期时间
    ...
    public static String createJWT(String subject, Long ttlMillis) {
        JwtBuilder builder = getJwtBuilder(ttlMillis, getUUID());// 设置过期时间
        return builder.compact();
    }

    private static JwtBuilder getJwtBuilder(Long ttlMillis, String uuid) {
    ...
    public static String createJWT(String id, String subject, Long ttlMillis) {
        JwtBuilder builder = getJwtBuilder(ttlMillis, id);// 设置过期时间
        return builder.compact();
    }
...
//        System.out.println(new BCryptPasswordEncoder().encode("laowu123"));
